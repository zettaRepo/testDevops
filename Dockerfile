...
# copy our app's files to apache's public directory
COPY ./app/ /var/www/html/
RUN  \
  echo "listen 192.168.1.41:80" >> /etc/httpd/conf/httpd.conf &&\
  echo "listen 192.168.1.42:80" >> /etc/httpd/conf/httpd.conf \
# keep apache server running
CMD exec /usr/sbin/httpd -D FOREGROUND -f /etc/httpd/conf/httpd.conf
